...
class _StatsGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cards = [
      SummaryCard(label: 'Total Income', value: Fmt.money(data.totalIncome),
          icon: Icons.trending_up, color: Colors.green),
      SummaryCard(label: 'Total Expenses', value: Fmt.money(data.totalExpenses),
          icon: Icons.trending_down, color: cs.error),
      SummaryCard(label: 'Monthly Collections', value: Fmt.money(data.monthlyTotal),
          icon: Icons.calendar_month, color: cs.primary),
      SummaryCard(label: 'Event Collections', value: Fmt.money(data.eventTotal),
          icon: Icons.celebration_outlined, color: Colors.purple),
      SummaryCard(label: 'Pending Monthly', value: '${data.pendingMonthly}',
          icon: Icons.pending_actions, color: Colors.orange),
      SummaryCard(label: 'Total Members', value: '${data.memberCount}',
...
class _RecentTransactions extends StatelessWidget {
  ...
  const _RecentTransactions(
      {required this.payments, required this.expenses, required this.ref});
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: _buildItems(db),
      builder: (ctx, snap) {
        if (!snap.hasData) return const LoadingView();
        final list = snap.data!;
        return Column(
          children: list
              .map((item) => _TxTile(item: item))
              .toList(),
        );
      },
    );
  ...
  Future<List<_TxItem>> _buildItems(AppDatabase db) async {
    final items = <_TxItem>[];
    for (final p in payments) {
      ...
      Collection? col;
      try {
        member = await db.getMemberById(p.memberId);
        col = await db.getCollectionById(p.collectionId);
      } catch (_) {}
      items.add(_TxItem(
        isExpense: false,
        amount: p.paidAmount,
    ...
    return Card(
      ...
      child: ListTile(
        ...
        leading: CircleAvatar(
          ...
          child: Icon(
              item.isExpense ? Icons.arrow_upward : Icons.arrow_downward,
              color: color,
              size: 16),
        ),
